...
@section('page-title', $registration->lead_name)
...
<div class="grid-2">
  <div class="card">
    ...
    <div class="card-body">
      <div class="detail-grid">
        <div class="detail-item"><div class="k">Participant</div><div class="v">{{ $registration->lead_name }}</div></div>
      ...
      <div style="margin-top:16px;display:flex;gap:8px;flex-wrap:wrap">
        <a href="mailto:{{ $registration->lead_email }}?subject=Masterclass%20Registration%20{{ $registration->reference }}" class="btn btn-primary btn-sm">Email participant</a>
  ...
  <div class="card">
    <div class="card-head"><h2>Institution</h2></div>
    <div class="card-body">
      <div class="detail-grid">
        <div class="detail-item full" style="grid-column:1/-1"><div class="k">School / institution</div><div class="v">{{ $registration->school_name }}</div></div>
...
<div class="grid-2">
  <div class="card">
    <div class="card-head"><h2>Registration fee</h2></div>
    <div class="card-body">
      <div class="detail-grid">
        <div class="detail-item"><div class="k">Fee type</div><div class="v">{{ $registration->tier === 'earlybird' ? 'School-Team Early Bird (legacy)' : 'Registration Fee' }}</div></div>
        <div class="detail-item"><div class="k">Rate</div><div class="v">KShs. {{ number_format($registration->rate_per_participant) }}</div></div>
        <div class="detail-item"><div class="k">Participants</div><div class="v">{{ $registration->participant_count }}</div></div>
        <div class="detail-item"><div class="k">Total due</div><div class="v" style="font-family:'Cormorant Garamond',serif;font-size:22px;color:var(--accent)">KShs. {{ number_format($registration->total_amount) }}</div></div>
      ...
      <form method="POST" action="{{ route('admin.registrations.update', $registration) }}">
        ...
        <div class="form-group" id="paymentMessageGroup">
          ...
          <p style="font-size:12px;color:var(--ink-soft);margin:0 0 8px;line-height:1.4;">
            Sent to the participant when status is set to <strong>Payment received</strong>.
...
@php $participant = $registration->participants->first(); @endphp
@if($participant && ($participant->subject || $participant->years))
<div class="card">
  <div class="card-head"><h2>Teaching profile</h2></div>
  <div class="card-body">
    <div class="detail-grid">
      @if($participant->subject)
        <div class="detail-item"><div class="k">Subject / grade</div><div class="v">{{ $participant->subject }}</div></div>
      @endif
      @if($participant->years)
        <div class="detail-item"><div class="k">Years of experience</div><div class="v">{{ $participant->years }}</div></div>
      @endif
    </div>
  </div>
</div>
